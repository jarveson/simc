...
struct ghoul_pet_t : public base_ghoul_pet_t
{
  ...
  ghoul_pet_t( death_knight_t* owner, bool guardian = true ) :
...
struct frost_fever_t final : public death_knight_disease_t
{
  ...
  frost_fever_t( util::string_view name, death_knight_t* p ) :
    ...
    rp_generation( as<int>( p -> spec.frost_fever -> effectN( 1 ).trigger() -> effectN( 1 ).resource( RESOURCE_RUNIC_POWER ) ) )
  {
  ...
  void execute() override
  {
    ...
    if ( ! p() -> buffs.dancing_rune_weapon -> up() )
    {
      p() -> pets.dancing_rune_weapon_pet.spawn( p() -> talent.blood.dancing_rune_weapon -> duration() +
                                                                               p() -> talent.blood.everlasting_bond -> effectN( 2 ).time_value(), 1 );
...
void death_knight_t::trigger_festering_wound_death( player_t* target )
{
  ...
  {
    for ( auto& ghoul : pets.ghoul_pet )
    {
      ghoul -> vile_infusion -> trigger();
    }
  }
}
...
void death_knight_t::burst_festering_wound( player_t* target, unsigned n )
{
  struct fs_burst_t : public event_t
  {
    ...
    void execute() override
    {
      ...
      {
        for ( auto& ghoul : dk->pets.ghoul_pet )
        {
          ghoul->vile_infusion->trigger();
        }
      }
...
void death_knight_t::apply_affecting_auras( action_t& action )
{
  ...
  action.apply_affecting_aura( sets->set( DEATH_KNIGHT_FROST, T29, B2 ) );
